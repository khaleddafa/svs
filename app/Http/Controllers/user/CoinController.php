...
class CoinController extends Controller
{
    ...
    public function buyCoinPage()
    {
        try {
            ...
            if (env('APP_ENV') == 'local') {
                $data['coins'] = Coin::where(['status' => STATUS_ACTIVE])
                    ->where('type', '<>', DEFAULT_COIN_TYPE)
                    ->get();
            } else {
                $data['coins'] = Coin::where(['status' => STATUS_ACTIVE])
                    ->whereNotIn('type', [DEFAULT_COIN_TYPE, COIN_TYPE_LTCT])
                    ->get();
            }
            ...
            $data['no_phase'] = $activePhases['status'] == false;

            if (!$data['no_phase'] && !$activePhases['futurePhase']) {
    ...
    public function buyCoin(btcDepositeRequest $request)
    {
        try {
            ...
            $url = file_get_contents('https://min-api.cryptocompare.com/data/price?fsym=USD&tsyms='.$baseCoin);
            $coinPrice = json_decode($url, true)[$baseCoin] ?? null;
            ...
            if ($coinPrice) {
                ...
                if (isset($request->phase_id)) {
                    $phase = IcoPhase::find($request->phase_id);
                    if ($phase) {
                ...
                $coin_price_btc = bcmul(custom_number_format($coinPrice), $coin_price_doller, 8);

                switch ($request->payment_type) {
                    case BTC:
                        $result = $coinRepo->buyCoinWithCoinPayment($request, $coin_amount, $coin_price_doller, $phase_id, $referral_level, $phase_fees, $bonus, $affiliation_percentage);
                        break;
                    case BANK_DEPOSIT:
                        $result = $coinRepo->buyCoinWithBank($request, $coin_amount, $coin_price_doller, $coin_price_btc, $phase_id, $referral_level, $phase_fees, $bonus, $affiliation_percentage);
                        break;
                    case STRIPE:
                        $result = $coinRepo->buyCoinWithStripe($request, $coin_amount, $coin_price_doller, $coin_price_btc, $phase_id, $referral_level, $phase_fees, $bonus, $affiliation_percentage);
                        break;
                    default:
    ...
    {
        $response = [];
        $response['title'] = __('Coin Payment');
        ...
        if (isset($coinAddress)) {
            $response['coinAddress'] = $coinAddress;
