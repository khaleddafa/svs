...
//        return view('user.faq.index', $data);
    ...
    public function showNotification(Request $request)
    {
        ...
        if (!$notification) {
            // Return a response indicating the notification was not found
            return response()->json(['success' => false, 'message' => 'Empty'], 404);
        }
    ...
    {
        $data['notifications'] = Notification::where(['user_id' => $request->user_id, 'status'=> STATUS_PENDING])->orderBy('id', 'desc')->get();
        return response()->json(['data' => $data]);
    }
}
